...
// Fonction pour formater une réponse HTTP
pub fn format_response(response: Response<Bytes>) -> Bytes {
    // Séparer la réponse en en-tête et corps
    let (head, body) = response.into_parts();
    let mut resp = Bytes::from(format!("{:?} {}\r\n", head.version, head.status));

    // Ajouter tous les en-têtes à la réponse
    ...
    resp.extend("\r\n".as_bytes()); // Ajouter le CRLF supplémentaire avant le corps de la réponse
    ...
    if is_chunked(head) {
        ...
        resp.extend("0\r\n\r\n".as_bytes()); // Fin des chunks
    } else {
        resp.extend(body); // Pas de chunks
    }

    resp
}

// Fonction pour vérifier si la réponse est en mode chunked
...
pub mod informational {
    ...
    // Fonction pour créer une réponse informative
    #[allow(dead_code)]
...
pub mod redirections {
    ...
    // Fonction pour vérifier si un chemin est une redirection
...
pub mod errors {
    ...
    // Fonction pour générer le contenu HTML d'une page d'erreur
...
<h1>{code} | {name}</h1>
    ...
    // Fonction pour vérifier et lire les pages d'erreur personnalisées
