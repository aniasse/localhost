...
impl ServerState<'_> {
    ...
    fn handle_timeout(&mut self) {
        ...
        // Supprimer les connexions qui ont expiré du `connections` HashMap
    ...
    if let Err(e) = crate::server::handle_connection(&mut connection.stream, &connection.config) {
        match e.kind() {
            ErrorKind::WouldBlock => {
                return; // Donc, nous gardons la connexion enregistrée et retournons
            }
...
// Fonction pour définir l'option linger sur un mio TcpStream
fn set_linger_option(stream: &TcpStream, linger_duration: Option<Duration>) -> std::io::Result<()> {
    ...
    std::mem::forget(socket); // Important pour éviter de fermer le descripteur de fichier
    Ok(())
}
